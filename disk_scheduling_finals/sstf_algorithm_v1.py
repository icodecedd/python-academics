...
def find_shortest_seek(active_track, last_track, track_queue):
    ...
    while track_queue:  # continue until all tracks are processed
        ...
        head_access_sequence.append(f"({active_track} - {nearest_track})")
    ...
    return total_head_movement, head_access_sequence, sorted_tracks


def plot_sstf(total_head_movement, head_access_sequence, sorted_tracks, seek_duration, active_track, screen):
    total_seek_duration = float(0)
    thm = " + ".join(map(str, head_access_sequence))
    ...
    panel_result = Panel(
        f"""
    [bold cyan]Total Head Movement:[/bold cyan] {thm} = {total_head_movement}
...
def main():
    ...
    total_head_movement, head_access_sequence, sorted_tracks = find_shortest_seek(active_track, last_track, track_queue)

    plot_sstf(total_head_movement, head_access_sequence, sorted_tracks, seek_duration, active_track, screen)
